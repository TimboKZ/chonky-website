...
Chonky supports drag & drop (D&D) out-of-the-box. The relevant functionality is
implemented using [react-dnd](https://react-dnd.github.io/react-dnd/docs/overview).

## Disabling D&D

If you want to disable D&D globally, you can use:
...
To disable D&D on a specific file browser, you can use:
...
For the majority of use cases, D&D will just work. However, when you start
using hot-reloading, or if you want to use multiple `FileBrowser` instances on the
same page, you will start seeing this error:
...
present somewhere in the component tree, above the component that actually uses D&D
